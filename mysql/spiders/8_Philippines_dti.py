...
class test_crawler(CrawlSpider):
    name = '8_Philippines_dti'
    allowed_domains = ['dti.gov.ph']
    ...
    key_name=['ocean']
    base_url='https://www.dti.gov.ph/component/search/?searchword={key}&ordering=newest&searchphrase=all&limit=30'
    ...
    def parse_pages(self, response):
        try:
            ...
            for news_url in response.xpath('//dl[@class="search-results"]/dt[@class="result-title"]/a/@href').extract():
                news_url="https://www.dti.gov.ph"+news_url
    ...
    def parse_news(self, response):
        try:
            ...
            #     item['image']=image
            # else:
            srcs = "https://www.dti.gov.ph"+"".join(
                response.xpath('//div[@class="item-page"]/div[@itemprop="articleBody"]/p/img/@src').extract())
            item['image_urls'] = srcs.split()
            #print("image:"+item['image'])
            item['source'] = "https://www.dti.gov.ph"

            item['content']="<br /><br />".join(response.xpath('//div[@class="item-page"]/div[@itemprop="articleBody"]/p/text()[name(..)!="img"]').extract())
            #print("content:"+item['content'])

            item['title']="".join(response.xpath('//*[@id="breadcrumbs"]/div/div/ul/li[5]/span/text()').extract())
            #print("title:" + item['title'])
            item['time']=None#"".join(response.xpath('//*[@id="content"]/div[2]/div[2]/div/p[1]/text()').extract())
