...
class test_crawler(CrawlSpider):
    name = '0_worldmaritimenews'
    allowed_domains = ['worldmaritimenews.com']
    key_name = ['China','Brunei','Cambodia','Indonesia','Lao','Malaysia',
          'Myanmar','Philippine','Singapore','Thailand','Vietnam' ]
    # key_name=['China']
    base_url={'https://search.worldmaritimenews.com/search?s={key}&submit=submit'
              # ,'https://search.worldmaritimenews.com/search?s={key}&page=2',
              # 'https://search.worldmaritimenews.com/search?s={key}&page=3',
              # 'https://search.worldmaritimenews.com/search?s={key}&page=4',
              # 'https://search.worldmaritimenews.com/search?s={key}&page=5',
              # 'https://search.worldmaritimenews.com/search?s={key}&page=6'
              }
    ...
    def parse_pages(self, response):
        try:
            ...
            #print(country)
            for news_url in response.xpath('//div[@class="search__result"]/a/@href').extract():
                news_url="https:"+news_url
                #print news_url.replace("/?p=", "/archives/");
    ...
    def parse_news(self, response):
        try:
            ...
            # else:
            #     item['image']=None
            srcs = "".join(response.xpath('//*[@id="post_thumbnail"]/a/@href').extract())
            item['image_urls']=srcs.split()

            item['content'] = "<br />".join(Selector(response=response).xpath('//div[@class="block-row"]/article/div[@class="content"]/p').extract())
            #[normalize-space()]

            item['title']="".join(response.xpath('//div[@class="block-row"]/article/header/h1/text()').extract())
            item['time']="".join(response.xpath('//div[@class="block-row"]/article/footer/p/text()').extract())
