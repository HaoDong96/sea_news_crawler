...
class test_crawler(CrawlSpider):
    name = '0_seatrade_maritime'
    allowed_domains = ['seatrade-maritime.com']
    key_name = ['China','Brunei','Cambodia','Indonesia','Lao','Malaysia',
          'Myanmar','Philippine','Singapore','Thailand','Vietnam' ]
    # key_name=['China']
    base_url={'http://www.seatrade-maritime.com/site-search.html?q={key}'
              # ,'http://www.seatrade-maritime.com/site-search.html?q={key}&start=20',
              # 'http://www.seatrade-maritime.com/site-search.html?q={key}&start=40',
              # 'http://www.seatrade-maritime.com/site-search.html?q={key}&start=60'
              }
    ...
    def parse_pages(self, response):
        try:
            ...
            for news_url in response.xpath('//ul[@class="search-results list-striped"]/li/h4/a/@href').extract():
                news_url="http://www.seatrade-maritime.com"+news_url
                print(news_url)
    ...
    def parse_news(self, response):
        try:
            ...
            item['content'] = "".join(response.xpath(
                    '//*[@id="k2Container"]/div[@class="itemBody"]/div[@class="contentBlock"]').extract())
            # print("content"+item['content'])

            item['title'] = "".join(
                    response.xpath('//*[@id="k2Container"]/div[@class="itemHeader"]/h1/text()').extract())
            item['time'] = "".join(
                    response.xpath('//*[@id="k2Container"]/div[@class="itemBody"]/span/text()').extract())
            item['crawled_time'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))

            srcs = "http://www.seatrade-maritime.com"+str("".join(response.xpath('//*[@id="k2Container"]/div[@class="itemBody"]/div[@class="itemImageBlock"]/span[@class="itemImage"]/a/@href').extract()))
